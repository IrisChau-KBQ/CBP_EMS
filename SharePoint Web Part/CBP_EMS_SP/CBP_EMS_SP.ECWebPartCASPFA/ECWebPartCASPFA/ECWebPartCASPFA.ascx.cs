...
namespace CBP_EMS_SP.ECWebPartCASPFA.ECWebPartCASPFA
{
    ...
    public partial class ECWebPartCASPFA : WebPart
    {
        ...
        private string regExAmount = @"^(?=.*\d)\d*(?:\.\d\d)?$";
        ...
        public TB_CASP_FINANCIAL_ASSISTANCE_REIMBURSEMENT objAppInit;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            ApplicantClaimedAmount = objAppInit.Total_Amount_After_Deduction.HasValue ? objAppInit.Total_Amount_After_Deduction.Value : 0;
            ...
            if (!Page.IsPostBack)
            {
                ...
                lblClaimedAmount.Text = objAppInit.Total_Amount_After_Deduction.HasValue ? "HK$" + objAppInit.Total_Amount_After_Deduction.Value.ToString("#,##0.00") : "HK$0.00";
            }
        ...
        protected TB_CASP_FINANCIAL_ASSISTANCE_REIMBURSEMENT GetUserApplication()
        {
            TB_CASP_FINANCIAL_ASSISTANCE_REIMBURSEMENT objApp = new TB_CASP_FINANCIAL_ASSISTANCE_REIMBURSEMENT();
            try
            {
                using (CyberportEMS_EDM dbContext = new CyberportEMS_EDM())
                {
                    Guid applicationId = Guid.Parse(m_ApplicationID);
                    objApp = dbContext.TB_CASP_FINANCIAL_ASSISTANCE_REIMBURSEMENT.FirstOrDefault(x => x.CASP_FA_ID == applicationId);
                }
                if (objApp == null)
                {
                    RedirectPageToBack();
        ...
        protected void AccessControl()
        {
            ...
            if (objAppInit.Actual_Claim_Amount.HasValue)
            {
                txtActualClaimAmount.Text = objAppInit.Actual_Claim_Amount.Value.ToString("F2");
                lblActualClaimAmount.Text = "HK$" + objAppInit.Actual_Claim_Amount.Value.ToString("#,##0.00");
            }
            ...
            if (m_Role == "CASP Coordinator")
            {
                ...
                else if (m_Status == stCEO_Approved || (m_Status == stFinance_Senior_Manger_Approved && ApplicantClaimedAmount <= 10000) || (m_Status == stCFO_Approved && ApplicantClaimedAmount <= 50000))
                {
                    ...
                    if (objAppInit.Deliver_Cheque_By == "By Coordinator")
                    {
                        ...
                        //BtnSubmit.ValidationGroup = reqCheque_DateFinance.ValidationGroup = regCheque_DateFinance.ValidationGroup = "Validate_ClaimInfo";//
        ...
        protected void btnsubmit_Click(object sender, EventArgs e)
        {
            ...
            if (m_Role == "Finance Accountant" && (m_Status == stFinance_Assist_Approved || m_Status == stFinance_Senior_Manger_Approved || m_Status == stCFO_Approved || m_Status == stSenior_Manager_Approved || m_Status == stCEO_Approved))
            {
                ...
                //if (Convert.ToDateTime(txtDeliver_Cheque_DateFinance.Text) < DateTime.Now)
                //    errlist.Add("Deliver Cheque Date should be future date.");
            }
        ...
        {
            TB_CASP_FINANCIAL_ASSISTANCE_REIMBURSEMENT objApp = GetUserApplication();
            ...
            using (CyberportEMS_EDM dbContext = new CyberportEMS_EDM())
            {
                objApp = dbContext.TB_CASP_FINANCIAL_ASSISTANCE_REIMBURSEMENT.FirstOrDefault(x => x.CASP_FA_ID == objApp.CASP_FA_ID);
        ...
        protected void getProgrmNameIntakeNumByProgID()
        {
            ...
            m_programName = "CASP Financial Assistance Reimbursement";
            m_intake = "";
